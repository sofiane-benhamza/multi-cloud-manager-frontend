...
export default function Header({ disconnect, username }) {
    ...
    return (
        <>
            <Navbar collapseOnSelect expand="lg" variant="dark" className="text-light tilt-warp-title h5 position-fixed w-100 bg-black" style={{ height: "80px", zIndex: "10", opacity: isNotAtTop ? 0.8 : 1 }}>
                <Link href={isLoggedIn ? "/home" : "/"} passHref legacyBehavior>
                    <Navbar.Brand>
                        <Image style= {{filter: "drop-shadow(0px 0px 1px #FFF)"}}src="/assets/images/logo.png" alt="Cloud logo" width={100} height={60} />
                    </Navbar.Brand>
                </Link>
