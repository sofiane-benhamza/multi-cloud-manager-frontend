...
function MyApp({ Component, pageProps }) {
  ...
  const disconnect = useCallback(async (reason) => { //if  reason === undefind => user disconnected // reason === expired => token is dead 
    try {
  ...
  useEffect(() => {
    if (restore) {
      ...
      const allowedPagesWhileDisconnected = ['/', '/login', '/signup','/404'];
      if (notConnected && !allowedPagesWhileDisconnected.includes(router.pathname)) {
        console.warn()
        router.push("/");
      }
      RestoreSession()
