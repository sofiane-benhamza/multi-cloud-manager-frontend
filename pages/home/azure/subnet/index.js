...
import { locations, getVNs, getResourceGroups } from "@/utils/azure";
...
export default function CreateSubnet({ setWarning, setToken }) {
    ...
    const subnetArray = Object.keys(subnet);
    const handleInputChange = (e) => {
        ...
        switch (name) {
            case "account":
                ...
                break;
            case "location":
                getResourceGroups(token, subnet.account, value, setChooseFrom) // Value = subnet.location
                break;
            case "resourceGroup":
                getVNs(token, subnet.account, subnet.location, setChooseFrom, value)  // Value = vm.resourceGroup
                break;
            default:
                break;
    ...
    const handleCreateSubnet = async (e) => {
        ...
        try {
            ...
            for (const key in subnet) {
                if (subnet.hasOwnProperty(key)) {
                    form.append(key, subnet[key]);
                }
            }

            const response = await fetch(
                `${process.env.NEXT_PUBLIC_BACKEND_ADDR}terraform/azure/subnet/`,
                {
                    method: "POST",
                    body: form,
                }
            ...
            setWarning({
                message: "something went wrong, please try again later or contact support [error : 723]",
                type: "danger",
                isShown: true
            });
        } finally {
                                ...
                                <div className="col-12">
                                    <form onSubmit={handleCreateSubnet}>
                                        ...
                                        <select
                                            ...
                                            required
                                        >
                                            <option value="" defaultValue disabled>Choose a virtual network</option>
