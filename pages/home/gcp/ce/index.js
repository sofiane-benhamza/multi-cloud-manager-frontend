...
import { regions, getCEs } from "@/utils/gcp";

export default function CE({ setWarning, setToken }) {
    ...
    useEffect(() => {
        if (filter.account && filter.region) {
            setFilter((prev) => ({ ...prev, instances: [{ name: wait, id: wait, status: wait, size: wait, privateIp: wait, publicIp: wait }] }))
            getCEs(token, filter.account, filter.region, setFilter).then((isOk) => {
                if (!isOk) {
                    setWarning({
                    ...
                    setFilter((prev) => ({ ...prev, instances: [{ name: "-", id: "-", status: "-", size: "-", privateIp: "-", publicIp: "-" }] }))
                }
            });
    ...
    //handle actions
    const handleCEAction = async (action, id, zone) => {
        ...
        actionConfig.append("zone", zone);


        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_BACKEND_ADDR}gcp/ce/`, {
                method: "PUT",   //update
                body: actionConfig,
            });
            if (response.ok) {
                setWarning({
                    message: `action ${action} has been applied on instance ${filter.instances.find(instance => instance.id === id)?.name || null}  successfully`,
                    type: "success",
                    isShown: true
                })
                setFilter(prev => ({
                    ...
                    instances: prev.instances.map(instance => {
                        if (instance.id === id) {
                            return { ...instance, status: "pending", publicIp: "N/A" };
                        }
                        return instance;
                    })
                }));
                        ...
                        <div className="card-body">
                            <div className="d-flex flex-row mb-4">
                                ...
                                <button className="border border-dark rounded d-flex align-items-center ml-auto mr-3 px-3 btn btn-light" onClick={() => {
                                    setFilter((prev) => ({ ...prev, instances: [{ name: wait, id: wait, status: wait, size: wait, privateIp: wait, publicIp: wait }] }))
                                    getCEs(token, filter.account, filter.region, setFilter)
                                    ...
                                    <tbody>
                                        {filter.instances.length > 0 ? (filter.instances.map((instance, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className="text-center">
                                                    <button className="btn mx-1 btn-success" title="Start" disabled={instance.status !== "TERMINATED"} onClick={() => { handleCEAction("start", instance.id, instance.zone) }}>
                                                        <i className="bi bi-play-fill"></i>
                                                    </button>
                                                    <button className="btn mx-1 btn-dark" title="Shutdown" disabled={instance.status !== "RUNNING"} onClick={() => { handleCEAction("stop", instance.id, instance.zone) }}>
                                                        <i className="bi bi-sign-stop"></i>
                                                    </button>
                                                    <button className="btn mx-1 btn-danger" title="Terminate" disabled={instance.status !== "TERMINATED" && instance.status !== "RUNNING"} onClick={() => { handleCEAction("terminate", instance.id, instance.zone) }}>
                                        ...
                                        <tr className="bg-dark">
                                            ...
                                            <td className="px-2">{ }</td>
                                            <td className="px-2">{ }</td>
                                            <td className="px-2 w-100 btn btn-success" onClick={() => { router.push('./ce/create'); }}><i className="bi bi-cloud-plus p-2"></i>Create a CE</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
