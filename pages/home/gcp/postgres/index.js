...
export default function EC2({ setWarning, setToken }) {
    ...
    useEffect(() => {
        if (filter.account && filter.region) {
            ...
            getSQLs(token, filter.account, filter.region, setFilter, "POSTGRES").then((isOk) => {
                if (!isOk) {
                    setWarning({
    ...
    const handleDBAction = async (action, id) => {
        ...
        try {
            ...
            if (response.ok) {
                setWarning({
                    message: `action ${action} has been applied on instance ${id}  successfully`,
                    type: "success",
                    isShown: true
                })
                setFilter(prev => ({
                        ...
                        <div className="card-body">
                            <div className="d-flex flex-row mb-4">
                                ...
                                <button className="border border-dark rounded d-flex align-items-center ml-auto mr-3 px-3 btn btn-light" onClick={() => {
                                    setFilter((prev) => ({ ...prev, instances: [{ name: wait, id: wait, status: wait, size: wait, privateIp: wait, gbs: wait }] }))
                                    getSQLs(token, filter.account, filter.region, setFilter, "POSTGRES")
                                ...
                                <table className="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        <tr className="bg-dark">
                                            ...
                                            <td className="px-2">{ }</td>
                                            <td className="px-2">{ }</td>
                                            <td className="px-2 w-100 btn btn-success" onClick={() => { router.push('./postgres/create'); }}><i className="bi bi-cloud-plus p-2"></i>Create a PostgrSQL DB</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
